...
function showPosition(position) {
    ...
    var map2 = "https://maps.googleapis.com/maps/api/staticmap?center="
    +latlon+"&zoom=14&size=400x300&key=AIzaSyBu-916DdpKAjTmJNIgngS6HL_kDIKU0aU";
    document.getElementById("mapholder").innerHTML = "<img src='"+map2+"'>";
...
function apiMarkerCreate(LatLng, placeResults) {
  ...
  if (placeResults) {
    var content = placeResult.name+'<br/>'+placeResult.vicinity+'<br/>'+placeResult.type+'<br/><a hrefs="addmap.php?name='+placeResult.name+'&adress'
      windowInfoCreate(marker, LatLng, content);
    }
    else {
      var content = 'You are here: '+ latLng.lat()
      + ', ' + latLng.lng();
     windowInfoCreate(marker, LatLng, content);  
